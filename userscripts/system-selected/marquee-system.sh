...
LOGFILE="/rcade/share/userscripts/system-selected/system-selected-debug.log"
if [ "$DEBUG" != "true" ]; then
  rm -f "$LOGFILE"
fi
#LOGFILE= "/dev/null"
echo "===========================================" >> $LOGFILE
echo "Script called at: $(date '+%Y-%m-%d %H:%M:%S.%3N')" >> $LOGFILE
echo "Number of arguments: $#" >> $LOGFILE
echo "Argument 1 (system): $1" >> $LOGFILE
...
echo "Sending command: SELECTED $system $game" >> $LOGFILE

# Send game info to picture server
python3 /rcade/share/userscripts/simpleClient.py "SELECTED $system" >> $LOGFILE 2>&1
#echo "SELECTED $system" > /tmp/marquee-daemon.pipe
